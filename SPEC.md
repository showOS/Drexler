...
- V74 — Bracketed-paste mode (`\x1b[?2004h`) is explicitly enabled on Ink mount when stdin is a TTY, disabled on unmount/SIGINT/SIGTERM (`\x1b[?2004l`). Best-effort: write failures swallowed; non-TTY launch (`§V23`) skips the toggle entirely.
- V75 — `/attach remove <n>` removes the 1-based n-th pending attachment (chip-strip order). Out-of-range or no pending attachments ⇒ local in-character notice, no state mutation.
- V76 — Multi-file drag/drop: when stdin payload is N newline-separated absolute paths (N ≥ 2) AND no chip-strip activity AND not slash, each path is fed to `loadAttachment` and successful results accrue up to remaining count/byte caps (§V69). Per-path failures emit one notice each; payload is not sent as text.
- V77 — Recent-files cache: ring buffer ≤ 10 most-recently-attached absolute paths, persisted at `~/.drexler/attach-recent.json` (atomic write per §V64). Reads tolerate missing/corrupt file (empty array fallback). Entries store path only — never filename mime / size / sha. Cache invalidates entries whose path no longer resolves to a regular file at chooser-render time.
- V78 — `/attach` slash with no argument opens an argument chooser sourced from the recent-files cache (V77). Empty cache ⇒ chooser shows a single hint row instead.
...
| T55 | x | Multi-file drag/drop: `parseMultiFileDrop()` in `src/attach/intake.ts` validates N≥2 newline-separated absolute paths. `onSubmit` consumes the raw payload (pre-trim) and loops `loadAttachment` per line; per-path failures emit one notice each, count-cap breach stops loop. Payload never sent as text. | V76,V68,V69 |
| T56 | x | `src/attach/recent.ts`: `loadRecent`, `loadRecentValid` (stale-path filter), `pushRecent(absPath)`. Persisted at `~/.drexler/attach-recent.json` via `withJsonFileLock` (§V64). Ring buffer capped at `MAX_RECENT_ENTRIES=10`. Wired into App.tsx at successful `/attach <path>`, single-file drop, and multi-file drop. | V77,V64 |
| T57 | x | `valuesForGroup()` in commands.ts injects up to 6 recent files (from `loadRecentValid()`) ahead of static `remove N` entries in the `/attach` chooser. Stale paths filtered at render time per §V77. ARGUMENT_BASE_NAMES auto-picks up `/attach` so palette opens chooser on Enter. | V78,V31,V62 |
| T58 | . | Explicit bracketed-paste mode toggle on Ink mount/unmount/SIGINT/SIGTERM. Write `\x1b[?2004h` on enable, `\x1b[?2004l` on disable. Skip non-TTY. Best-effort writes; never throw. | V74,V23 |
| T59 | . | Bracketed-paste modal: instead of auto-attach + notice, render a confirm/decline modal (`PasteIntakePrompt`) for large/binary pastes — Enter attaches, ESC discards, `i` inserts as plain text (override). Modal owns input until resolved. Honors §V22 + §V31. | V70 |
