...
| T58 | x | `src/attach/bracketedPaste.ts`: `enableBracketedPaste`/`disableBracketedPaste` (`\x1b[?2004h`/`\x1b[?2004l`) + `installBracketedPasteSignalHandlers` (SIGINT/SIGTERM/exit). All writes guarded by `process.stdout.isTTY` per §V23; failures swallowed. Wired into Ink mount/unmount in App.tsx. | V74,V23 |
