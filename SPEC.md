...
| T54 | . | `/attach remove <n>` slash + argument chooser sourced from current chips. Removes 1-based n-th pending attachment, prints confirmation; out-of-range = local notice, no state change. | V75,V62 |
| T55 | . | Multi-file drag/drop: in `onSubmit`, when stdin payload is N≥2 newline-separated absolute paths AND no chips pending AND not slash, loop `loadAttachment` per line, accrue up to caps, one notice per failure. | V76,V68,V69 |
| T56 | . | Recent-files cache `~/.drexler/attach-recent.json` via `withJsonFileLock` (§V64). `src/attach/recent.ts` exports `loadRecent`, `pushRecent(path)`. `tryAttach` success path hooks `pushRecent`. Reads tolerate missing/corrupt. | V77,V64 |
| T57 | . | `/attach` no-arg argument chooser sourced from recent-files cache (V77). Filter out paths that no longer resolve to regular file at render time. Empty cache ⇒ hint row only. Wire into ARGUMENT_PALETTE + isArgumentParentCommand. | V78,V31,V62 |
